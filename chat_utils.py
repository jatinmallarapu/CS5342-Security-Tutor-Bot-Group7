...
def get_bot_response(user_query, current_chat):
    network_security_keywords = [
        'encryption', 'aes', 'des', 'rsa', 'cryptography', 'firewall', 'vpn',
        'ssl', 'tls', 'hash', 'authentication', 'authorization', 'malware',
        'virus', 'attack', 'security', 'network', 'protocol', 'tcp', 'ip',
        'packet', 'cipher', 'key', 'certificate', 'vulnerability', 'exploit',
        'blockchain', 'bitcoin', 'digital signature', 'public key', 'private key'
    ]
    
    query_lower = user_query.lower()
    is_network_security_query = any(keyword in query_lower for keyword in network_security_keywords)
